...
(function(window, undefined) {
						...
						for (p = 0; p < props.length; p++) {
							...
							if (typeof q === OBJ_TYPE && q.length > 0) {
								if (q.length == 2) {
									...
									this[q[0]] = match ? q[3].call(this, match.replace(q[1], q[2])) :
										undefined
								}
							} else {
	...
	var regexes = {
		...
		device: [
			...
			[/(archos)\s(gamepad2?)/i, /(hp).+(touchpad)/i, /(hp).+(tablet)/i, /(kindle)\/([\w\.]+)/i,
		...
		os: [
			...
			[NAME, VERSION],
			[/(windows)\snt\s6\.2;\s(arm)/i, /(windows\sphone(?:\sos)*)[\s\/]?([\d\.\s\w]*)/i,
	...
	var UAParser = function(uastring, extensions) {
		...
		var ua = uastring || (window && window.navigator && window.navigator.userAgent ? window.navigator
			.userAgent : EMPTY);
